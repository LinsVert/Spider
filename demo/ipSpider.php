...
//可以使用redis队列 将验证通过的ip按时间push进去

// 然后 此脚本用于 抓取数据 入队

//可以做个队列时间池

//本脚本需要做的就是 隔段时间抓取代理网站的ip 并且验证ip可用，然后入列

//如何做到 短时间内验证完 这些ip 并且 可以让爬虫可用

//想到的解决方法是 两个脚本，一个抓完验证，另一个则是一直验证，失效丢弃
...
function fork_one_task($i,iPool $iPool,$redisKey){
    $pid = pcntl_fork();
    if(0 < $pid){
        //todo
    }elseif (0 == $pid){
        echo "Fork $i Success! Pid is ".posix_getpid().PHP_EOL;
        switch ($i){
            case 0:
                $ips = $iPool->get66Ip(100);
            break;
            case 1:
                $ips = $iPool->getjxlIpH();
            break;
...
function check_ip($ips){
    ...
    foreach ($ips as $key){
        ...
        if (!requests::$error && $html) {
            $successIp[] = $key;
            $end = microtime(1);
            echo 'SuccessIp '.$key.' Speed = '.($end-$start) .'s'.PHP_EOL;
        }else {
            requests::$error = '';
        }
